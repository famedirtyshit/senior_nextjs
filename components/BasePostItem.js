...
export default function BasePostItem(prop) {
    ...
    return (
        <div onClick={emitPostData} className="post-item cursor-pointer">
            <p className="text-right text-textGray text-sm font-medium">{prop.data.post != undefined ? moment(new Date(prop.data.post.createdAt)).fromNow() : moment(new Date(prop.data.createdAt)).fromNow()}</p>
            ...
            <div className="post-item-desc lg:ml-4">
